...
- `lib/actions/billing.ts` — merchant billing server actions (`saveBillingDetailsAction`, `payNowAction`), role-gated to `owner`/`admin`/`finance`; audit-logs field names only, never the phone/PII.
