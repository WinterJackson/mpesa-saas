...
## Billing & pricing files (Phase 5)
- `lib/pricing.ts` — the SINGLE pricing catalog (pure, no prisma). `SEED_PLANS` in `lib/repositories/billing.ts` derives from `SEEDABLE_TIERS` here, and the public `/pricing` page + its estimator read it, so marketing display and DB billing can never drift. Pricing is FLAT-FEE (monthly plan + flat KES overage per transaction) — NEVER a percentage of a merchant's sale.
- `lib/billing/platform-mpesa.ts` — the platform-as-collector STK path: PaySwift charges merchants' subscription fees INTO its OWN shortcode. Deliberately separate from the per-merchant Model-B path in `lib/daraja.ts` (a distinct actor, not a tenant). Env-driven (`PLATFORM_BILLING_*`) with pooled-sandbox fallback; never logs a secret.
- `app/api/mpesa/billing/callback/route.ts` — the SOLE writer of subscription-invoice terminal status (mirrors the merchant STK / B2C result callbacks). Idempotent; ResultCode 0 → paid + reactivate; non-zero → failed + past_due w/ grace. Always returns 200.
- `lib/billing/dunning.ts` — the PURE dunning policy (max attempts, retry spacing, grace before soft-lock). `lib/billing/subscription-billing.ts` — `chargeInvoice` (never throws; typed reasons) + `runDunningCycle` (retries/suspends; never writes terminal status).
- `app/api/cron/process-billing/route.ts` — dunning cron (cron-job.org, gated by `lib/cron-auth.ts`). `app/api/cron/aggregate-usage/route.ts` fires the first charge post-response.
- `lib/actions/billing.ts` — merchant billing server actions (`saveBillingDetailsAction`, `payNowAction`, `changePlanAction`), role-gated to `owner`/`admin`/`finance`; audit-logs field names only, never the phone/PII. `changePlanAction` never writes terminal invoice status — it issues a fresh invoice and delegates collection to `chargeInvoice` + the billing callback (guardrail #29).
- `lib/billing/tax.ts` — pure Kenyan VAT computation (VAT-inclusive split), env-gated on `PLATFORM_VAT_REGISTERED`. `lib/billing/etims.ts` — the KRA eTIMS OSCU provider seam; active provider is `NullEtimsProvider` (never fabricates a CU number); `getEtimsProvider()` is the single swap point for a certified integrator. `lib/billing/invoice-pdf.ts` — pdf-lib invoice renderer (serverless-safe; QR-ready).
...
## Phase 5 guardrails (billing & pricing)
24. **Pricing is FLAT-FEE, never a % of GMV.** A plan is a monthly fee + a flat `overageFeeKes` per transaction beyond `includedTransactions`. `lib/pricing.ts` is the single catalog; `SEED_PLANS` derives from it and `computeInvoiceAmount` is the one costing formula (reused by the usage cron, the merchant projection, and the public estimator). Never reintroduce basis-points/`txFeeBps`.
25. **Subscription collection is the platform-as-collector path.** PaySwift charges merchants INTO its own shortcode via `lib/billing/platform-mpesa.ts` (env-gated `PLATFORM_BILLING_*`, pooled-sandbox fallback) — never a merchant's Model-B credentials. The billing STK callback (`app/api/mpesa/billing/callback`) is the SOLE writer of subscription-invoice terminal status; the dunning runner and cron NEVER write paid/failed. A failed STK callback IS authoritative for that attempt (unlike `querySTKPushStatus`).
26. **Dunning surfaces, then soft-locks — it never deletes.** Retry with backoff up to the cap, hold a grace window, then set the subscription `suspended` (reversible). Marking an invoice paid (merchant Pay-now callback OR admin `billing:write` override) reactivates a dunned subscription.
27. **eTIMS/VAT: honest interim until VAT-registered.** While `PLATFORM_VAT_REGISTERED !== 'true'`, invoices are interim non-tax documents — VAT 0, NO fabricated eTIMS/CU number, a clear disclaimer. Never invent a KRA PIN or control-unit number. The recognized path is OSCU via a KRA-certified integrator behind `lib/billing/etims.ts`; `getEtimsProvider()` is the only swap point (currently `NullEtimsProvider`).
28. **Billing mutations are role-gated.** Merchant-side (`lib/actions/billing.ts`): `owner`/`admin`/`finance` only. Admin-side (`mark-paid`): the `billing:write` capability. Audit-log billing changes with field names/ids only — never the phone/PII/secret values.
29. **Paid plans are pay-first; the free tier is immediate.** Picking a plan on `/pricing` carries through sign-up (`/sign-up?plan=…` → `/onboarding?plan=…` → `setup`). A FREE plan (Starter, `monthlyFee` 0) activates immediately; a PAID plan (Growth/Scale) is created via `ensureSubscriptionForPlan` in a NEW `incomplete` subscription status WITH a first-period invoice, and only becomes `active` when the billing STK callback settles that invoice (`ResultCode 0`). A never-activated `incomplete` sub STAYS incomplete on a failed first payment — no `past_due`/grace window (there is no access to protect); the outstanding invoice keeps it gated. `incomplete` is excluded from MRR and the renewal cron. Self-service plan changes (`changePlanAction`) reuse the same rule: downgrade-to-free is immediate; switch-to-paid issues a fresh invoice and requires payment unless the org is already an active payer. Access is soft-gated: the dashboard stays usable behind a persistent "activate" banner (`components/billing/dashboard-billing-banner.tsx`) until paid. Never introduce a second subscription-activation path that bypasses the billing callback.
