  ...
  return (
    <>
      ...
      <div className="hidden md:flex h-full shrink-0 pt-floating-header pl-floating-header pb-floating-header">
        <aside className="flex flex-col w-64 border border-primary rounded-floating-header bg-sidebar text-sidebar-foreground shadow-floating-header overflow-hidden">
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 px-floating-header pb-[calc(var(--spacing-floating-header)+env(safe-area-inset-bottom))]">
        <nav className="flex items-center justify-around border border-primary rounded-floating-header bg-floating-header-bg backdrop-blur-md shadow-floating-header p-3">
          {navItems.map((item) => {
            const isActive = pathname === item.href;
            return (
              <Link
